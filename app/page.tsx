        ...
        <div className="mt-10 mx-auto max-w-2xl">
          <div className="rounded-3xl bg-white/85 shadow-[0_10px_30px_rgba(0,0,0,.08)] ring-1 ring-emerald-200/50 backdrop-blur">
            ...
            <div className="px-6 pb-5 text-sm text-gray-600 text-left">
              例：
              <button type="button" onClick={()=>router.push("/search?motif=サルート+蝶")}
                className="ml-1 underline decoration-emerald-300 hover:decoration-emerald-500">
                サルート 蝶
              </button>
              <span>、</span>
              <button type="button" onClick={()=>router.push("/search?motif=孔雀+グリーン")}
        ...
        <div className="mt-8 flex flex-wrap justify-center gap-2.5">
          {quickTags.map((tag) => (
            <button key={tag}
              ...
              className={`rounded-full px-3.5 py-1.5 text-sm shadow-sm transition hover:-translate-y-0.5 ${
                tag === "黒×金"
